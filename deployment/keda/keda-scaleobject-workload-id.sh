...
if [ -z "$WORKLOAD_MANAGED_IDENTITY_NAME" ]; then
  echo ${RED} "WORKLOAD_MANAGED_IDENTITY_NAME is not set. Make sure you have run deployment/environmentVariables.sh" ${NC}
  exit 1
else
  if ! az identity show -g $RESOURCE_GROUP -n $WORKLOAD_MANAGED_IDENTITY_NAME --query clientId -o tsv >/dev/null; then
    echo ${RED} "Workload Identity $WORKLOAD_MANAGED_IDENTITY_NAME does not exist in resource group $AZURE_RESOURCE_GROUP. Did you run the deployment script?" ${NC}
    exit 1
  else
    echo "Getting workload identity client id for $WORKLOAD_MANAGED_IDENTITY_NAME"
    workloadManagedIdentityClientId=$(az identity show -g $RESOURCE_GROUP -n $WORKLOAD_MANAGED_IDENTITY_NAME --query clientId -o tsv)
  fi
fi
